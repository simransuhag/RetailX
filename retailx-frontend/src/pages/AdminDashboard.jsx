...
import { Card, CardContent } from "../Components/ui/card"; // Check C or c
    ...
    <div className="min-h-screen bg-[#FDFDFD] flex font-sans text-slate-900">
      ...
      <aside className="w-64 border-r border-slate-200/60 flex flex-col sticky top-0 h-screen bg-white">
        ...
        <nav className="flex-1 px-4 space-y-1">
          ...
          <SidebarItem icon={Users} label="Sellers" active={active === "Seller"} onClick={() => setActive("Seller")} />
      ...
      <main className="flex-1 flex flex-col">
        <header className="h-20 border-b border-slate-100 px-10 flex items-center justify-between sticky top-0 bg-white/90 backdrop-blur-md z-10">
          ...
          <div className="flex items-center gap-6">
            <div className="relative p-2 hover:bg-slate-50 rounded-full cursor-pointer">
        ...
        <div className="p-12 max-w-7xl w-full mx-auto">
          <div className="flex items-center justify-between mb-12">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-2 font-medium">Platform performance overview.</p>
            </div>
            <Button className="h-12 px-6 text-sm font-bold rounded-xl bg-[#059669] hover:bg-[#10b981] text-white transition-transform active:scale-95">
              <Plus size={18} className="mr-2" /> Add New {active}
            </Button>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {(active === "users" || active === "Seller") && <UsersView />}
            </motion.div>
          </AnimatePresence>
        </div>
      </main>
...
// SidebarItem and StatCard helper components (Using your original logic for values)
function SidebarItem({ icon: Icon, label, active, onClick }) {
  return (
    <div onClick={onClick} className={`flex items-center gap-4 px-4 py-3.5 rounded-xl cursor-pointer transition-all text-sm font-bold ${active ? "bg-emerald-50 text-emerald-700 shadow-sm border border-emerald-100" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"}`}>
...
function StatCard({ label, value, trend, up }) {
  return (
    <Card className="border-none shadow-sm rounded-2xl bg-white p-8 group hover:shadow-xl transition-all duration-500">
    ...
    <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
      <table className="w-full text-sm text-left">
        <thead className="bg-slate-50 border-b border-slate-100">
          <tr>
            <th className="px-8 py-5 font-bold text-slate-500 uppercase text-[11px]">User Account</th>
            <th className="px-8 py-5 font-bold text-slate-500 uppercase text-[11px] text-center">Status</th>
            <th className="px-8 py-5 font-bold text-slate-500 uppercase text-[11px] text-right">Settings</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {[{ n: "Amit Sharma", e: "amit@retailx.com" }].map((u, i) => (
            <tr key={i} className="hover:bg-slate-50/50 transition-colors">
              <td className="px-8 py-6">
                <div className="flex items-center gap-4">
                  <div className="h-10 w-10 bg-slate-900 rounded-xl text-white flex items-center justify-center font-bold text-xs">{u.n[0]}</div>
                  <div><p className="font-bold text-slate-900">{u.n}</p><p className="text-xs text-slate-400">{u.e}</p></div>
                </div>
              </td>
              <td className="px-8 py-6 text-center"><span className="px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 font-bold text-[10px]">Verified</span></td>
              <td className="px-8 py-6 text-right"><button className="text-xs font-bold text-indigo-600 px-4 py-2 hover:bg-indigo-50 rounded-lg">Manage</button></td>
            </tr>
          ))}
        </tbody>
      </table>
