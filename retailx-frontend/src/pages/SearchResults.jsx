...
export default function SearchResults() {
  ...
  const handleAddToCart = async (p) => {
    const productForCart = {
      _id: p.id || p._id,
      name: p.name,
      price: Number(p.finalPrice), // Discounted price hi cart mein jayega
      ...
      <div className="pt-24 max-w-7xl mx-auto px-6 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <h1 className="text-xl md:text-2xl font-semibold text-gray-800">
            {category ? `Category: ${category}` : `Results for "${query}"`}
            <span className="text-gray-400 font-normal ml-2">({filteredProducts.length} items)</span>
          </h1>

          <div className="flex flex-wrap gap-4 items-center">
            <select className="bg-white border border-gray-200 px-3 py-2 rounded-lg text-sm outline-none" onChange={(e) => setSortBy(e.target.value)}>
              <option value="">Sort By</option>
              <option value="low-to-high">Price: Low to High</option>
              <option value="high-to-low">Price: High to Low</option>
            </select>

            <select className="bg-white border border-gray-200 px-3 py-2 rounded-lg text-sm outline-none" onChange={(e) => setSelectedBrand(e.target.value)}>
              {brands.map(brand => <option key={brand} value={brand}>{brand}</option>)}
            </select>

            <div className="flex items-center gap-2 bg-white border border-gray-200 px-3 py-2 rounded-lg text-sm">
              <span>Max: ₹{maxPrice}</span>
              <input type="range" min="500" max="100000" step="500" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)} className="cursor-pointer" />
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {filteredProducts.length > 0 ? (
                filteredProducts.slice(0, visibleCount).map((p) => (
                  <div key={p.id || p._id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col">
                    ...
                    <Link to={`/product/${p.id || p._id}`} className="block">
                      <div className="h-56 bg-white p-4 flex items-center justify-center relative">
                        ...
                        {/* Deals Badge (Optional: Extra visual for active deals) */}
                        {p.has_deal && (
                      ...
                      <div className="p-4 border-t border-gray-50 flex-grow">
                        ...
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          ...
                          {/* YAHAN DEALS KA LOGIC HAI: Agar has_deal true hai toh purana price dikhao */}
                          {p.has_deal && (
                            <>
                    ...
                    <div className="px-4 pb-4 mt-auto">
                      <button 
                        ...
                        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm"
                      >
                        Add to Cart
                      </button>
                    </div>
