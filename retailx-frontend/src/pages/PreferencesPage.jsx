...
export default function PreferencesPage() {
 ...
 const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await fetch("http://localhost:5000/api/preferences", { // 127.0.0.1 ki jagah localhost behtar hai
      method: "POST",
      headers: {
    ...
    // ✅ Match exactly what CustomerDashboard expects
    localStorage.setItem("user_preferences", JSON.stringify(selectedCategories));
    
    // Agar backend user ka naam bhi update kar raha hai ya bhej raha hai:
    if (data.user && data.user.name) {
        localStorage.setItem("user_name", data.user.name);
    }

    // ✅ Final Redirect to Customer Dashboard
